...
export class Movie extends Connection{
    ...
    async getMoviesByActorId({id}={id: 1}) {
        await this.client.connect();
        const data = await this.collection.find({"character.id_actor": id}).project({name: 1}).toArray();
        await this.client.close();
        return data;
    }
    ...
    async getMoviesWithMainActors() {
        await this.client.connect()
        const data = await this.collection.find({"character.rol": 'principal'}).project({name: true}).toArray()
        await this.client.close()
        return data 
    }
